...
class Database:
    ...
    def get_dish_id(self, name: str, menu_id: int = None) -> int:
        ...
        if menu_id is None:
            ...
            return self.execute(
                "SELECT ID FROM plats WHERE NAME = %s AND MENU = %s",
                (name, menu_id)
            )
    ...
    def add_review(self, restaurant_id: int, client_id: int, rating: int, comment: str, recommendation: Recommendation, plat: list[str], price: int, begin: int, end: int, date_rating: str, physical_note: int = None, delivery_note: int = None):
        ...
        if not self.is_owner(client_id):
            review_id = self.execute(
                ...
                recommendation.int(), price, physical_note, delivery_note, begin, end, date_rating)
            )[0]
            for elem in plat:
                self.execute(
    ...
    def delete_review_anonymously(self, review_id: int, mod_comment: str):
        ...
        self.execute(
            ...
            (id, restaurant_id, client_id, date_rating, comment, rating, recommendation, price, note_physical, note_delivery, begin_hour, end_hour, mod_comment)
        )
        self.execute(
